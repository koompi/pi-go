...
func usage() {
	fmt.Println(`Usage:
	pi = help
	pi -ud = check for updates
	pi -ug = check and install updates
	pi -S <app-name> = install app-name
	Example: 
	pi -S firefox = install firefox
...
func displayNumberMenu(pkgS []string) (err error) {
	...
	default:
		return fmt.Errorf("Invalid Sort Mode. Fix with pi -Y --bottomup --save")
	}
