...
func (parser *arguments) parseCommandLine() (err error) {
	...
	// fmt.Println(upd)
	// fmt.Printf("%T\n", upd)
	// fmt.Println(reflect.DeepEqual(args, upd[1:]))

	if len(args) < 1 {
		parser.parseShortOption("-h", "")
	} else if reflect.DeepEqual(args, upd[1:]) {
		parser.parseShortOption("-Sy", "")
	} else if reflect.DeepEqual(args, upg[1:]) {
	...
	cmdArgs.extractpiOptions()
	return
}

func (parser *arguments) extractpiOptions() {
