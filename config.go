...
// Configuration stores pi's config.
...
// configfile holds pi config file path.
var configFile string

// vcsfile holds pi vcs info file path.
var vcsFile string

// shouldSaveConfig holds whether or not the config should be saved
var shouldSaveConfig bool

// piConf holds the current config values for pi.
var config *Configuration

// AlpmConf holds the current config values for pacman.
var pacmanConf *pacmanconf.Config

// AlpmHandle is the alpm handle used by pi.
var alpmHandle *alpm.Handle

// Mode is used to restrict pi to AUR or repo only modes
var mode = modeAny

var hideMenus = false

// SaveConfig writes pi config to file.
...
func defaultSettings() *Configuration {
	...
	if os.Getenv("XDG_CACHE_HOME") != "" {
		config.BuildDir = "$XDG_CACHE_HOME/pi"
	}
